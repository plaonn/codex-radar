...
## VS Code Webview Dashboard Contract

현재 GUI surface는 VS Code extension의 Codex Radar Activity Bar container 안에 있는 단일 Webview dashboard다. 이 dashboard는 extension host-local `codex-radar` state를 읽어 프로젝트 단위 conversation list, attention inbox, selected-session inspector, hidden-session restore surface를 제공한다. Remote SSH 사용 시 기본 관심사는 VS Code UI client machine의 Codex state가 아니라 remote workspace extension host 안의 Codex state와 transcript다.
...
- VS Code extension은 Explorer 하위 view가 아니라 dedicated Codex Radar Activity Bar container 안에 Webview dashboard를 제공한다.
...
- Dashboard는 `Attention`, project-grouped session list, selected-session inspector를 같은 surface에 보여준다.
- `Attention`은 cross-project inbox이며 `waiting_approval`, `stale`, unread `done` session을 모은다.
- Hidden session은 `Attention`과 project list에서 제외되지만 dashboard 안의 hidden list에서 선택하고 restore할 수 있다.
- Session card와 inspector는 readable title/snippet, scan-friendly status text, read/unread state, current tool/model/relative last-seen metadata를 표시한다. 기본 dashboard snippet은 session cache의 redacted latest assistant summary를 사용하고 raw transcript file을 row마다 읽지 않는다.
- GUI는 `waiting_approval`, `running`, `tool_running`, `done`, `stale`를 dashboard 안에서 구분한다.
...
- GUI는 attention-worthy session 수를 dashboard topbar와 attention pane count 같은 in-surface cue로 보여준다. 이 count는 현재 status filter와 무관하게 전체 loaded visible session 기준으로 계산한다.
...
- GUI는 done session에 extension-local read/unread state를 유지할 수 있다. read key는 `session_id`와 done `last_seen_at` 기준이며, 같은 session이 다시 done 상태로 갱신되면 새 unread item으로 취급한다. Done card와 inspector action은 unread/read 상태를 구분해야 하며, read/unread action은 hide/show를 암시하는 eye icon이나 wording을 쓰지 않는다.
...
- 현재 VS Code Webview dashboard는 Python core CLI를 실행하지 않는다. retention config/prune GUI는 clearer surface가 생길 때 별도 requirement로 재도입한다.
...
- VS Code extension은 transcript preview action을 제공하지 않는다. 자세한 transcript 확인은 official Codex handoff 또는 terminal `codex-radar transcript` workflow에 맡긴다.
...
- 현재 테스트는 hook event normalization/cache update, no-default event log, config/pruning, stale display status, session filters, transcript skim/redaction, TUI project grouping, TUI resume guard, status watcher, shell completion, VS Code Webview dashboard view model/manifest를 보호한다.
