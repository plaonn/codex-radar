...
- 주요 future notification surface는 VS Code extension notification이다.
- `codex-radar watch`는 terminal에서 직접 실행하는 MVP/fallback 알림 표면으로 유지하며, 최종 VS Code notification 요구사항을 충족한 것으로 보지 않는다.
