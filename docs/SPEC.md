...
- VS Code extension은 filter가 없을 때 root에 `Attention` group을 먼저 표시해 attention session을 cross-project inbox처럼 모은다. 그 아래에는 기존 project group을 유지한다. `Attention` group의 session row description은 project를 함께 보여준다.
