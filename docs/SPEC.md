...
- `codex-radar export preview <session-id> --limit <n> [--contract-version <1|2>]`: 명시한 indexed session 하나의 host-local transcript를 resolve해 bounded, redacted transcript-preview contract만 stdout에 출력한다. `n`은 1 이상 200 이하로 명시해야 한다. Version을 생략하면 기존 client 호환을 위해 immutable v1을 출력하고, v2 client는 `--contract-version 2`를 명시해 optional per-message `timestamp`를 협상한다.
...
- Preview v2의 optional `timestamp`는 raw rollout JSONL record의 top-level timezone-aware `timestamp`에서만 가져와 UTC ISO-8601로 sanitize한다. Missing, timezone-naive, malformed, nested timestamp는 생략한다. 같은 role/text가 adjacent wrapper에 중복되면 message는 하나로 합치고 duplicate들 중 가장 이른 valid timestamp를 유지한다.
...
- Sidebar `Projects` Webview는 project group navigation을 유지하고, project header를 fold/unfold 가능한 navigation row로 표시한다. VS Code workspace folder와 raw `session.cwd`가 같거나 하위 path인 current workspace project group은 sidebar `Projects` 최상단에 고정하고, 여러 workspace folder가 match되면 workspace folder 순서대로 고정한다. Workspace 판정은 sanitized card 변환 전에 수행하며 raw `session.cwd`가 명시적으로 다른 path를 가리키면 project name만으로 current workspace로 분류하지 않는다. Raw `session.cwd`가 없으면 workspace folder basename과 project name 일치를 fallback으로 사용할 수 있다. Current workspace project group은 기본 expanded 상태를 유지하고 header 안에 `Current Workspace` label을 2단으로 표시한다. 나머지 project group은 기존처럼 최신 session 순서를 따른다. Status filter가 없을 때 attention이 없는 quiet project는 기본 collapsed 상태로 시작할 수 있고, filter가 켜져 있으면 matching session을 바로 확인할 수 있도록 project groups는 펼쳐진다.
...
- GUI는 valid exact session id가 있는 sidebar/dashboard/preview session에 공식 URI handoff와 구분되는 `Open in Codex CLI` action을 제공한다. 사용자가 이 action을 실행하면 extension host의 user-visible VS Code integrated terminal을 `codexRadar.codexExecutable`의 structured `shellPath`와 exact `["resume", session_id]` `shellArgs`로 생성한다. Raw `session.cwd`가 있으면 terminal `cwd`로 사용하고, 없으면 terminal default working directory로 fallback한다. Shell command string을 조합하거나 status/client를 추론하지 않으며, official `Open in Codex` workspace mismatch behavior를 변경하지 않는다. VS Code가 terminal을 extension host에 생성하므로 local, Remote SSH, WSL, Dev Container ownership semantics도 같은 API boundary를 따른다.
...
- GUI의 local command execution은 사용자가 명시적으로 누른 `Open in Codex CLI` integrated-terminal action으로 제한한다. Background resume, automatic retry/fork, shell-string execution은 수행하지 않는다. 공식 Codex extension URI handoff는 별도 experimental action으로 유지한다.
- VS Code GUI의 기본 list source는 `codex-radar export state --json` shared sanitized display-state contract다. Export command 실패, source unavailable, schema mismatch 시 direct `sessions.json` adapter로 fallback한다. `observe`와 `direct` mode는 진단/호환 목적으로 유지한다. Raw `cwd`와 transcript path는 sanitized contract에 넣지 않고 workspace handoff, 명시적 preview, fallback에 필요한 trusted extension-host adapter metadata로만 결합한다. 명시적인 preview 동작은 bounded transcript-preview v2를 명시적으로 협상하는 `export preview`를 우선 호출하고 direct transcript preview를 fallback으로 유지한다. Direct, observe, export preview adapter는 top-level timestamp sanitization과 adjacent duplicate merge에 같은 의미를 사용한다. Retention config/prune GUI는 clearer surface가 생길 때 별도 requirement로 재도입한다.
...
- Current App Server Controller는 optional read-only metadata enrichment와 supported usage lookup만 담당한다. Python hook/helper producer를 제거하거나 대체하지 않으며, lifecycle attention parity와 local/Remote SSH migration gate는 유지한다. Native Windows real-host producer-to-sidebar proof는 helper `0.4.8` + VSIX `0.4.18` exact pair에서 완료됐고, 이후 Windows 회귀 검증과 release-scoped support 확대는 변경 surface 및 별도 release decision에 따른다. Direct stdio child의 abrupt extension-host-loss cleanup은 supported cross-platform guarantee가 아니며, daemon/proxy 전환은 public platform, socket security/auth, ownership, crash/stale cleanup, version compatibility contract가 생기기 전까지 distribution 방향이 아니다.
...
- 사용자가 VS Code GUI의 `Open in Codex CLI` action을 실행했을 때 extension host의 user-visible integrated terminal에서 configured Codex executable을 exact `resume`, session id argv로 실행하고 available raw session cwd를 사용하기.
