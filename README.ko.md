...
현재 source extension에는 선택적인 thread title/archive 보강을 위한 읽기 전용 Codex App Server Controller foundation도 포함되어 있습니다. Controller는 사용자가 별도로 설치한 compatible Codex CLI를 실행하며, VSIX는 Codex를 번들하거나 공식 Codex extension의 private runtime 경로를 재사용하지 않습니다. App-server lifecycle parity와 실제 환경 migration smoke가 끝날 때까지 Python helper/indexer는 계속 필요합니다.
