...
- Preview v2의 optional `recorded_at`은 raw rollout JSONL record의 top-level timezone-aware `timestamp`에서만 가져와 UTC ISO-8601로 sanitize한다. Missing, timezone-naive, malformed, nested timestamp는 생략한다. 같은 role/text가 adjacent wrapper에 중복되면 message는 하나로 합치고 duplicate들 중 가장 이른 valid timestamp를 유지한다.
