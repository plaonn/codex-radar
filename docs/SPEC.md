...
- Current App Server Controller는 optional read-only metadata enrichment만 담당한다. Python hook/helper producer를 제거하거나 대체하지 않으며, lifecycle attention parity와 local/Remote SSH/Native Windows smoke가 별도 migration gate로 남는다.
