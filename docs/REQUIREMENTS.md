...
- Derived specs/tests: [Android foreground cockpit MVP contract](proposals/android-foreground-cockpit-mvp.md), packaged read-only `codex-radar mobile rpc`, shared mobile/gui display-state and preview contracts, strict stdout protocol with diagnostics on stderr, bounded/redacted preview output, foreground in-app attention events, explicit SSH host-key verification, and no remote HTTP listener by default; Stage 0 transport semantics are proven by the shared Python core, compatibility spike, deterministic `tests/fixtures/mobile-rpc-v1.json`, protocol tests, and disposable reconnect loopback. Android A2 adds the isolated Kotlin/platform-Views fixture cockpit, mechanical display-state/preview fixture derivation and drift detection, memory-only bounded preview, foreground-baseline attention UI, JVM tests, and emulator UI/privacy-negative checks without live SSH. UX-EXEC-1 adds explicit connection/UI state and virtualized rows, one state-aware primary connection action including foreground `연결 재개`, privacy-separated connection details and blocking host review, `확인 필요 → 진행 중 → 프로젝트` home navigation with Archived separated, a dedicated explicit-preview thread detail, and serialized foreground `attention/poll → state/read` reconciliation with generation-invalidated lifecycle callbacks.
...
- Requirement: VS Code extension은 사용자가 별도로 설치한 compatible Codex CLI를 통해 `codex app-server`를 실행하고, extension host 안의 Codex App Server Controller가 process lifecycle, initialization, request routing, timeout, failure recovery를 관리할 수 있어야 한다. 정상 deactivation/reload와 extension-host abrupt loss의 process-lifetime 보장을 구분하고, supported contract가 없는 crash cleanup을 구현된 보장으로 과장하지 않아야 한다.
...
- Failure prevented: Radar가 Codex binary 또는 별도 app-server implementation을 소유·번들하는 것으로 굳어지는 문제, official Codex extension의 private executable path 의존, refresh마다 process를 생성하는 비용과 process leak, ownership 증명 없는 PPID/process-name sweep으로 다른 Codex client를 종료하는 문제, app-server parity 검증 전에 Python producer를 제거하는 문제.
- Assumptions: compatible `codex` executable은 extension host에 사용자가 별도로 설치하며, stdio app-server surface를 제공한다. Radar가 실행한 app-server process는 다른 Codex client의 process와 독립적일 수 있지만 같은 host-local Codex state를 읽는다. 정상 extension deactivation/reload에서는 controller `dispose()`가 child를 정리하지만, extension host가 deactivation 없이 종료되면 direct stdio child가 남을 수 있다.
- Non-goals: Codex binary 번들, official Codex extension 내부 runtime 재사용, `turn/start` 또는 Codex state-changing method 활성화, 현재 Python hook/helper 즉시 제거, app-server만으로 lifecycle attention parity가 확보됐다고 선언하기, private/undocumented daemon·proxy 자동 bootstrap, process-name/PPID 기반 orphan sweep, supported contract 없이 cross-platform crash cleanup 완료를 선언하기.
- Derived specs/tests: `CodexAppServerController`, configurable executable path, lazy start and single-process reuse, initialize/initialized handshake, request id routing, timeout/exit rejection, reset/dispose cleanup, read-only active/archived `thread/list`, read-only `account/rateLimits/read`, controller/catalog/usage unit tests, packaged VSIX without a Codex binary, disposable normal-shutdown/reload/extension-host-loss process-lifecycle smoke.
- Revisit when: supported app-server event/status contract가 `waiting_approval`, `tool_running`, `done`, transcript preview, usage를 current producer와 동등하게 제공하거나, Codex CLI가 explicit parent-lifetime/stdio EOF contract를 제공하거나, public daemon/proxy contract가 macOS/Linux/Windows support, per-user socket ACL/auth, Remote SSH ownership, crash/stale cleanup, version compatibility를 명시할 때.
