...
- GUI는 host-local Codex usage snapshot을 VS Code status bar에 표시할 수 있다. 표시 대상은 primary/secondary remaining percent이며, 예시는 `29% · 89%` 형태다. Used percent, reset time, plan, latest token summary는 tooltip과 click detail에 둘 수 있다. Usage unavailable 상태는 오류로 띄우지 않고 muted status item으로 표시한다.
