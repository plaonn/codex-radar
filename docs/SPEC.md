...
- VS Code extension은 Codex Radar Activity Bar container 안에 `Attention`, `Projects`, `Hidden` native TreeView sections를 제공한다. `Attention`은 attention session을 cross-project inbox처럼 모으고, `Projects`는 기존 project group navigation을 유지하며, `Hidden`은 사용자가 숨긴 session을 collapsed section으로 보여준다. `Attention`과 `Hidden` section의 session row description은 project를 함께 보여준다.
...
- GUI는 `display_status` 기준으로 session list를 좁히는 read-only status filter를 제공할 수 있다. `attention` filter는 display status가 아니라 attention state 기준으로 좁힌다. 첫 구현은 view-local temporary filter로 두며, session cache나 extension settings를 수정하지 않는다.
