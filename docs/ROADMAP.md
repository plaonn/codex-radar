...
- Status: `active`; A1 packaged host read protocol is complete in helper runtime `0.4.11`, while Android A2 remains unclaimed.
...
- Ordered implementation packages:
  1. complete: productize the Stage 0 semantics as `codex-radar mobile rpc` in helper runtime `0.4.11`, without changing R12 `thread rpc`;
