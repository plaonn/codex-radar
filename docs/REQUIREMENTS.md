...
- Derived specs/tests: planned GUI read contract v1, project-grouped navigation, in-surface attention cues, read-only cache-change refresh, read-only first milestone, extension surface boundary, `extensions/vscode` scaffold and SessionSource tests.
...
- Derived specs/tests: GUI project grouping rules, direct `sessions.json` read contract v1, `extensions/vscode` SessionSource grouping tests, future extension view tests.
