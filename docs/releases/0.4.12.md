...
Codex Radar 0.4.12 packages the post-0.4.4 reliability and host-integration work as a GitHub Release public beta. The release pairs VSIX `0.4.12` with helper runtime `0.4.7`; the two versions remain intentionally independent.
...
The published candidate was checked with the following release gate:
...
Codex Radar `0.4.12`는 `0.4.4` 이후의 안정성 개선과 host 통합 작업을 GitHub Release 공개 베타로 묶습니다. 이 release는 VSIX `0.4.12`와 helper runtime `0.4.7`을 함께 제공하며, 두 버전은 의도적으로 독립적으로 관리됩니다.
...
게시된 GitHub Release에서 다음 순서로 설치합니다.
