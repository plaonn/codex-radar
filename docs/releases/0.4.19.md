...
Codex Radar `0.4.19`는 게시된 `v0.4.12` 공개 베타 이후의 확장 작업을 다음 공개 베타로 묶습니다. 이 prerelease는 VSIX `0.4.19`와 helper runtime `0.4.10`을 함께 제공하며 두 version은 의도적으로 독립적으로 관리됩니다.
...
지원되는 POSIX Python 3.9+ host에서 bundle/checksum을 검증하고 helper를 설치한 뒤, hook migration을 먼저 no-write로 검토합니다. `hook-config --apply`는 사용자가 명시적으로 선택한 경우에만 실행합니다. 같은 local 또는 Remote SSH extension host에 `codex-radar-vscode-0.4.19.vsix`를 설치합니다.
