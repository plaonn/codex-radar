...
- Provides native sidebar sections for `Attention`, `Projects`, and collapsed `Hidden`.
...
- Distinguishes unread/read done sessions in sidebar rows and the dashboard selected-session inspector.
...
- Opens a sidebar row or dashboard-selected session in the official Codex extension via `vscode://openai.chatgpt/local/<session_id>`.
...
```bash
code --install-extension extensions/vscode/codex-radar-vscode-0.2.1.vsix --force
```
...
4. Confirm the sidebar shows native `Attention`, `Projects`, and collapsed `Hidden` sections.
5. Confirm sidebar rows show status, model/tool metadata, and redacted snippets from `sessions.json`.
...
9. On a done session, mark read and unread from either sidebar row actions or the dashboard inspector.
