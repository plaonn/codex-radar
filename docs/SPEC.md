...
- GUI는 done session에 extension-local read/unread state를 유지할 수 있다. read key는 `session_id`와 done `last_seen_at` 기준이며, 같은 session이 다시 done 상태로 갱신되면 새 unread item으로 취급한다. 이 read/unread state는 `sessions.json` source cache에서 더 이상 보이지 않는 done session의 read key를 refresh 시 정리해 source-cache-bounded derived UI state로 유지한다. Done row/card와 inspector action은 unread/read 상태를 구분해야 하며, read/unread action은 hide/show를 암시하는 eye icon이나 wording을 쓰지 않는다.
