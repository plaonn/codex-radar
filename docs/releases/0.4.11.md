# Codex Radar 0.4.11 Public Beta Candidate

Codex Radar 0.4.11 packages the post-0.4.4 reliability and host-integration work as the next GitHub Release candidate. The candidate pairs VSIX `0.4.11` with helper runtime `0.4.6`; the two versions remain intentionally independent.
...
- `codex-radar-helper-0.4.6.zip`
- `codex-radar-helper-0.4.6.zip.sha256`
- `codex-radar-vscode-0.4.11.vsix`

The helper bundle includes a pure-Python `0.4.6` wheel and compatibility metadata for VS Code extension versions from `0.4.3` up to, but not including, `0.5.0`.
...
2. Verify the helper bundle:
   ...
   ```bash
   sha256sum --check codex-radar-helper-0.4.6.zip.sha256
   ```

3. Install or upgrade the helper runtime:

   ```bash
   unzip codex-radar-helper-0.4.6.zip
   cd codex-radar-helper-0.4.6
...
5. Install the VSIX on the same local or Remote SSH extension host:
   ...
   ```bash
   code --install-extension codex-radar-vscode-0.4.11.vsix --force
   ```
...
- Build and inspect the `0.4.6` wheel and helper bundle.
- Build and inspect the `0.4.11` VSIX.
...
Codex Radar `0.4.11`은 `0.4.4` 이후의 안정성 개선과 host 통합 작업을 다음 GitHub Release 후보로 묶습니다. 이 후보는 VSIX `0.4.11`과 helper runtime `0.4.6`을 함께 제공하며, 두 버전은 의도적으로 독립적으로 관리됩니다.
...
Helper bundle에는 pure-Python `0.4.6` wheel과 VS Code extension `0.4.3` 이상 `0.5.0` 미만을 대상으로 하는 compatibility metadata가 포함됩니다.
...
- `0.4.6` wheel과 helper bundle을 생성하고 검사합니다.
- `0.4.11` VSIX를 생성하고 검사합니다.
