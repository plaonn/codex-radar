...
- Done session 중 transcript path가 비어 있거나 resolve되지 않고, host-local Codex transcript store에서도 session id로 transcript file을 찾지 못하며, Codex app-server catalog와 `state_5.sqlite` thread state에서도 exact `session_id` row를 찾지 못하는 session은 unresolvable done session으로 취급해 `Attention`과 active project list에서 제외한다. 이 경우 archived로 표시하지 않고, app-server catalog lookup이 실패해 missing 여부가 불확실하면 숨기지 않는다.
...
- GUI는 attention-worthy session 수를 sidebar view badge, dashboard topbar, attention pane count, Radar-native VS Code status bar item 같은 in-surface cue로 보여준다. Status bar item은 attention, running/tool_running, visible session counts를 existing read-only session model에서 계산한다. 이 attention count는 현재 status filter와 무관하게 전체 loaded visible session 기준으로 계산한다.
...
- GUI는 done session에 extension-local read/unread state를 유지할 수 있다. read key는 `session_id`와 done `last_seen_at` 기준이며, 같은 session이 다시 done 상태로 갱신되면 새 unread item으로 취급한다. 이 read/unread state는 `sessions.json` 원천 세션 캐시에서 더 이상 보이지 않는 done session의 read key를 refresh 시 정리해 원천 캐시 기준으로 제한되는 파생 UI 상태로 유지한다. Done row/card와 inspector action은 unread/read 상태를 구분해야 하며, read/unread action은 hide/show를 암시하는 eye icon이나 wording을 쓰지 않는다.
